...
function PlayPageClient() {
  ...
  // 🚀 提取fetchSourcesData作为组件级函数（从useEffect中提取）
  ...
  useEffect(() => {
    const fetchSourceDetail = async (
      source: string,
      id: string,
      shouldSearchAllSources = false // 新增参数：是否搜索所有源
    ): Promise<SearchResult[]> => {
      try {
        let detailResponse;
        ...
        // 如果需要搜索所有源（用于一键测速功能），则进行搜索
        if (shouldSearchAllSources && detailData.title) {
          console.log('🔍 搜索所有可用源，用于一键测速功能');
          const allSources = await fetchSourcesData(detailData.title);
          // 确保当前源在列表中
          if (!allSources.some((s) => s.source === source && s.id === id)) {
            allSources.unshift(detailData);
          }
