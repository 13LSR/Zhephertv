...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (aiRecommendConfig.enabled) {
      if (
        !aiRecommendConfig.apiUrl ||
        typeof aiRecommendConfig.apiUrl !== 'string'
      ) {
        return NextResponse.json({ error: 'API地址不能为空' }, { status: 400 });
      }

      if (
        !aiRecommendConfig.apiKey ||
        typeof aiRecommendConfig.apiKey !== 'string'
      ) {
        return NextResponse.json({ error: 'API密钥不能为空' }, { status: 400 });
      }

      if (
        !aiRecommendConfig.model ||
        typeof aiRecommendConfig.model !== 'string'
      ) {
        return NextResponse.json(
          { error: '模型名称不能为空' },
          { status: 400 }
      ...
      if (
        typeof aiRecommendConfig.temperature !== 'number' ||
        aiRecommendConfig.temperature < 0 ||
        aiRecommendConfig.temperature > 2
      ) {
        return NextResponse.json(
          { error: '温度参数应在0-2之间' },
          { status: 400 }
      ...
      if (
        !Number.isInteger(aiRecommendConfig.maxTokens) ||
        aiRecommendConfig.maxTokens < 1 ||
        aiRecommendConfig.maxTokens > 150000
      ) {
        return NextResponse.json(
          {
            error:
